 ...
 * second is theta(radent) that value are read from encoder
 *
 */
...
//			(4*R*x1 + 4*p11*theta + 4*dt_pow2*p22*theta + 4*R*dt*x2 + 4*dt*p12*theta + 4*dt*p21*theta + Q*dt_pow4*theta)/(4*R + 4*p11 + 4*dt*p12 + 4*dt*p21 + Q*dt_pow4 + 4*dt_pow2*p22);
	kalman->x2 = x2 - (((Q*(dt_pow3))/2 + p22*dt + p21)*(x1 - theta + x2*dt))/(p11 + R + p21*dt + (Q*(dt_pow4))/4 + dt*(p12 + p22*dt));
//			x2 - (((Q*dt_pow3)/2 + p22*dt + p21)*(x1 - theta + dt*x2))/(R + p11 + dt*p21 + (Q*dt_pow4)/4 + dt*(p12 + dt*p22));
...
//			(R*(4*p11 + 4*dt*p12 + 4*dt*p21 + Q*dt_pow4 + 4*dt_pow2*p22))/(4*R + 4*p11 + 4*dt*p12 + 4*dt*p21 + Q*dt_pow4 + 4*dt_pow2*p22);
	kalman->p12 = -((p11 + p21*dt + (Q*(dt_pow4))/4 + dt*(p12 + p22*dt))/(p11 + R + p21*dt + (Q*(dt_pow4))/4 + dt*(p12 + p22*dt)) - 1)*((Q*(dt_pow3))/2 + p22*dt + p12);
//			(2*R*(Q*dt_pow3 + 2*p22*dt + 2*p12))/(4*R + 4*p11 + 4*dt*p12 + 4*dt*p21 + Q*dt_pow4 + 4*dt_pow2*p22);
